...
def map_to_result(batch, processor=None, model=None, group_ids=None):
    ...
    for i in range(len(group_ids)):
        ...
        pred.append((processor.batch_decode(pred_ids,spaces_between_special_tokens=True)[0],pred_ids))
    ...
    if isinstance(results, DatasetDict):
        for dataset in results:
            for g in range(ngroups):
                pred = [item[g][0] for item in results[dataset]['pred_str']]
        ...
        for g in range(ngroups):
            pred = [item[g][0] for item in results['pred_str'][0]]
